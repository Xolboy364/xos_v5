"""
╔══════════════════════════════════════════════════════════════════╗
║          ZAFAR TILI v2.0 — xCPU-1 bilan TO'LIQ BIRIKKAN        ║
╠══════════════════════════════════════════════════════════════════╣
║  Sintaksis:                                                      ║
║    son x = 5          → LOAD X0, #5                             ║
...
class ZafarXato(Exception):
    def __init__(self, xabar, satr=None, ustun=None):
        self.satr  = satr
        self.ustun = ustun
        joy = f" [satr {satr}]" if satr else ""
        super().__init__(f"⚠ Zafar xatosi{joy}: {xabar}")

class ZafarChipXato(Exception):
...
class Lexer:
    ...
    def _qator(self, matn, satr_n):
        ...
        while i < n:
            ...
            if c == '"':
                ...
                while j < n and matn[j] != '"':
                    if matn[j] == '\\' and j+1 < n:
                        ...
                        j += 1
                if j >= n:
                    raise ZafarXato("Matn yopilmagan (\" yetishmaydi)", satr_n)
                raw = matn[i+1:j]
                # Escape sequences
                val = raw.replace('\\n', '\n').replace('\\t', '\t').replace('\\"', '"')
            ...
            raise ZafarXato(f"Noma'lum belgi: '{c}'", satr_n)
...
class Parser:
    ...
    def _buyruq(self):
        ...
        raise ZafarXato(f"Kutilmagan token: '{t.qiymat}'", t.satr)
    ...
    def _id_buyruq(self):
        ...
        raise ZafarXato(f"'{nom}' dan keyin '=' yoki '(' kerak", satr)
    ...
    def _birlamchi(self):
        ...
        raise ZafarXato(f"Ifoda kutilgan, '{t.qiymat}' topildi", t.satr)
...
class Doira:
    ...
    def top(self, nom):
        ...
        raise ZafarXato(f"'{nom}' o'zgaruvchisi topilmadi")
...
class Compiler:
    ...
    def kompil(self, dastur):
        self._e(
            "; ═══════════════════════════════════════════════════",
            "; Zafar tili v2.0 — xCPU-1 chiqishi",
    ...
    def _tugun(self, t):
        if   isinstance(t, OzgarT):  self._ozgar(t)
            ...
            raise ZafarXato(f"Noma'lum tugun: {type(t).__name__}")
    ...
    def _ifoda(self, tugun, maq=REG_T1):
        ...
        raise ZafarXato(f"Noma'lum ifoda: {type(tugun).__name__}")
    ...
    def _binary(self, t, maq):
        ...
        if op == TT.PLUS:
            ...
            raise ZafarXato(f"Noma'lum operator: '{op}'")
        return maq
    ...
    def _toxta(self):
        if not self._loop_stek:
            raise ZafarXato("'to'xta' faqat 'takror' ichida ishlatiladi")
        _, lbl_oxir = self._loop_stek[-1]
        self._e(f"    JMP {lbl_oxir}  ; to'xta")

    def _davom(self):
        if not self._loop_stek:
            raise ZafarXato("'davom' faqat 'takror' ichida ishlatiladi")
...
class Zafar:
    """
    Zafar tilini ishlatish uchun bosh sinf.
    ...
    def run(self, matn, machine=None, max_cycles=500_000):
        """
        Zafar kodini xOS xCPU-1 da ishlatish.
        ...
        if machine is None:
            try:
                _zafar_dir = os.path.dirname(os.path.abspath(__file__))
                _xos_root  = os.path.dirname(_zafar_dir)
                _src_root  = os.path.dirname(_xos_root)
                ...
                try:
                    _zafar_dir = os.path.dirname(os.path.abspath(__file__))
                    _root = os.path.dirname(_zafar_dir)
...
def testlar_o_tkaz():
    ...
    o_tildi = xato = 0
    print("╔══════════════════════════════════════════════════════╗")
    print("║     Zafar v2.0 × xCPU-1 — Integratsiya Testlari     ║")
    print("╠══════════════════════════════════════════════════════╣")
